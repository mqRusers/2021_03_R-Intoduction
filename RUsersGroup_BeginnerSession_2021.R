...
#  Beginner Session V1"-file. If you have it already installed both, pls make
...
#  4. Basic statistics.
#  5. Plotting.
#  6. Not being scared of coding and getting stuck!
...
#     If we watned to access that result later, we need to save it as a new object
b <- mean(x)
#     You can find out more about what your objects are by using the structure
#     fuction: str()
str(a)
str(b)
str(x)  # these are all numeric vectors
...
#     Check it!
seq_a <-
  seq(1, 13, by = 1)  # once you know which areguments are required where,
...
# or we can "read" in the data from a file on our computer:

irisdata <-
  read.csv("input/irisdata.csv")  #this works becuase we've set up a
# project,if we weren't using a project we would need to write out the full file path.
# or use setwd() , which sets the wording directory to a particlar path.

irisdata #  prints out the data (or a subset of the data for LARGE datasets)
# .csv - comma separated file - why?
...
as.matrix(irisdata)$Sepal.Length # this won't work for matrices. atomic vectors =
# (logical, integer, double (sometimes called numeric), and character)
...
####  2. Clean the data ####
# up a bit and specify that the group is a factor variable.
